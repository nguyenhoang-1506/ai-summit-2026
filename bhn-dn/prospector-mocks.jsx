...
function ChatScreen({ T, from, to, at, part }) {
  ...
  const ZC = FRAME_H / 880, msgs = CHATS[part], W = Math.round(FRAME_W / ZC), H = 920, L = (DEMO_X + (FRAME_W - W * ZC) / 2) / ZC, Tp = (FRAME_TOP - 20) / ZC;
  const shown = msgs.filter(m => T >= at + m.off);
  const next = msgs.find(m => T < at + m.off);
  const typing = next && next.who === 'ai' && T >= at + next.off - 0.8;
  return <div style={{ position: 'absolute', left: L, top: Tp, width: W, height: H, zoom: ZC, borderRadius: 28, background: '#F7F8FC', boxShadow: '0 28px 64px rgba(0,0,0,.45)', overflow: 'hidden', display: 'flex', flexDirection: 'column', fontFamily: UI, opacity: op, transform: `translateY(${(1 - rp) * 30}px)` }}>
